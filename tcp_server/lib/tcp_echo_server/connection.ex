defmodule TCPEchoServer.Connection do
  ...
  def init(socket) do
    Logger.info(
      "Step 4 <Connection Process>: initializing new connection managed by process #{inspect(self())}."
    )
  ...
  def handle_info(
    ...
    :ok = :inet.setopts(socket, active: :once) # Active once socket
  ...
  def handle_info({:tcp_closed, socket}, %__MODULE__{socket: socket} = state) do
    Logger.info("TCP Closed for process #{inspect(self())}")
    {:stop, :normal, state}
  end
